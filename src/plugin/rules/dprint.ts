...
import { ConfigSchema } from '../schema/dprint';
import { createRule } from '../utils/eslint';
import * as RegularExpression from '../utils/regexp';
  ...
  switch (diff.operation) {
    ...
    case 'replace': {
      if (
        ...
        const oldHasLinebreak = RegularExpression.hasLineBreak(
          diff.deleteText!,
        );
        const newHasLinebreak = RegularExpression.hasLineBreak(
          diff.insertText!,
        );
      ...
      for (const diff of diffIterator) {
        ...
        context.report({
          ...
          fix(fixer) {
            switch (diff.operation) {
              case 'insert': {
                return fixer.insertTextAfterRange(range, diff.insertText ?? '');
              }
